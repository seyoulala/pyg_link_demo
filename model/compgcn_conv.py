...
class CompGCNConv(MessagePassing):
    def __init__(self, in_channels, out_channels, num_rels, act=lambda x: x, params=None):
        ...
        self.w_loop = get_param((in_channels, out_channels))  # self loop 边的weight
        self.w_in = get_param((in_channels, out_channels))  # 入边的weight
        self.w_out = get_param((in_channels, out_channels))  # 出边的weight
        self.w_rel = get_param((in_channels, out_channels))  # 关系的weight，用来对边的embedding进行转换
        self.loop_rel = get_param((1, in_channels))  # self loop 这条边的embedding
