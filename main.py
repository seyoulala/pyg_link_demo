    ...
    with th.no_grad():
        ...
        for step, (triples, trp_ids) in tqdm(enumerate(test_iter)):
            ...
            for (triple, pred, triple_id) in zip(triples, preds, ids):
                ...
                train_set.update(data.sr2o['valid'][(s, r, r1)])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--feature_method', dest='feature_method', default='sum', type=str,
                        help='Feature combine method')
    parser.add_argument('--lb_smooth', dest='lb_smooth', default=0, type=float, help='label smooth')
    parser.add_argument('--add_parent_rel', dest='add_parent_rel', action='store_true',
                        help='Whether add parent relaiton')
